...
def direction_process_d1(imgpath, savedir):
    if not os.path.exists(savedir):
        os.mkdir(savedir)
    assert os.path.exists(imgpath), f"please check your dataPath: {imgpath}"
...
def main():
    ...
    parser.add_argument('-d', '--base_dir', type=str, default=r'D:\MyWorkSpace\dl_dataset\road_extraction\masa\test\png',
        help='Base directory for Spacenent Dataset.')
    args = parser.parse_args()

    gt_path = args.base_dir
    fileName = gt_path.split("\\")[-1]
    connect_d1_path = gt_path.replace(fileName, "train_connect_8_d1")
    connect_d3_path = gt_path.replace(fileName, "train_connect_8_d3")
    # connect_d1_path = os.path.join(args.base_dir, 'train_connect_8_d1')
    # connect_d3_path = os.path.join(args.base_dir, 'train_connect_8_d3')
