...
def calculate_weigths_labels(classes_weights_path, dataloader, num_classes):
...
class Trainer(object):
    def __init__(self, cfg):
        ...
        classes_weights_path = cfg.train_root.replace("images", 'classes_weights.npy')
        if cfg.weight:
            if os.path.exists(classes_weights_path):
                weight = np.load(classes_weights_path)
            else:
                train_loader, valid_loader = self.build_loader()
                weight = calculate_weigths_labels(classes_weights_path, train_loader, cfg.num_classes)
            weight = torch.from_numpy(weight.astype(np.float32))
        else:
            weight = None

        self.criterion = dice_bce_loss()
        self.criterion_con = SegmentationLosses(weight=weight, cuda=self.device).build_loss(mode="con_ce")
